...
> 1.0.0 更新
> `seju-lite` 已提升为正式版 `1.0.0`。
> 这次上下文管线向更轻量的 v2 方向收敛：长期记忆不再直接整段注入，而是先整理成更紧凑的结构化视图再注入 prompt。
> 这次升级 context / memory 的直接原因，是长期会话在持续积累后，会让上下文构建越来越重，进而拉长模型响应时间；同时 HISTORY.md 在长期使用中也会不断堆积低价值、重复性的内容，导致记忆检索和后续注入越来越冗杂。
<!-- > 本次更新主要希望解决三个痛点：第一，短期历史越聊越厚，连简单问候和轻量请求都会被旧上下文拖慢；第二，长期记忆与会话历史边界不清，很多本该只属于临时对话的信息被反复带入后续请求；第三，长期使用后 memory/history 噪声增加，真正有价值的稳定信息反而不够突出。
> 对应的解决方案是：将长期记忆从“整段原文注入”改为“压缩后的结构化视图注入”，优先保留更稳定、更高价值的 profile / current focus / stable facts；同时在构建短期上下文前增加轻量 history 去噪，过滤重复寒暄、身份确认等低信号消息；整体上把 session context 和 long-term memory 的职责分开，让上下文更短、更干净、更适合长期运行。
> 这次更新不会改变 seju-lite 轻量自定义 runtime 的总体方向，但会让它在长期使用场景下更稳定：构建上下文更可控，输出延迟更容易维持在合理范围内，记忆系统也更接近生产级产品中“短期会话”和“长期记忆”分层治理的思路。 -->
